    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        applicationScope.launch {
            ...
            for (message in messages) {
                ...
                val amount = service.extractAmount(content)?.toDoubleOrNull()
                val merchant = service.extractMerchant(content)

                println("AppDebug: Extracted Amount - $amount")
                println("AppDebug: Extracted Merchant - $merchant")
                ...
                /*notificationHelper.postNotification(
                    id = insertedId.toInt(),
                    title = "Expense Added",
                    content = "Amount $amount spent at $merchant"
                )*/
            }
        }
    }
